...
def get_compound_pair_dis_distribution(coords, LAS_distance_constraint_mask=None):
    ...
    if LAS_distance_constraint_mask is not None:
        ...
        for i in range(pair_dis.shape[0]):
            pair_dis[i, i] = 0      # 对角线为0
    pair_dis = torch.tensor(pair_dis, dtype=torch.float)
    pair_dis[pair_dis>bin_max] = bin_max
    pair_dis_bin_index = torch.div(pair_dis - bin_min, bin_size, rounding_mode='floor').long() #torch.div表示除法
...
def get_plm_reps(df, model, converter):
    seq = df['seq']# ''.join([RESTYPE_3to1(i) for i in list(df['resname'])])
...
def pocket_in_protein(res_list,pro_res_list):
    res_list = [res for res in res_list if (('N' in res) and ('CA' in res) and ('C' in res) and ('O' in res))]
    pro_res_list = [res for res in pro_res_list if (('N' in res) and ('CA' in res) and ('C' in res) and ('O' in res))]
    protein = {}
    protein['name'] = 'protein'
    protein['seq'] = "".join([three_to_one.get(res.resname) for res in pro_res_list])
    index = []
    i = 0
    ca_coord = []
    for pro_res in pro_res_list:
        pro_atom = pro_res['CA']
        ca_coord.append(pro_atom.coord)
        for res in res_list:
            for atom in [res['CA']]:
                if (atom.coord[0] == pro_atom.coord[0]) and (atom.coord[1] == pro_atom.coord[1]) and (atom.coord[2] == pro_atom.coord[2]):
                    # print("pocket coord is {}, protein coord is{}, they are equal.".format(atom.coord, pro_atom.coord))
                    index.append(i)
        i += 1
    protein['CA'] = ca_coord
